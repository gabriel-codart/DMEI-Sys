...
// Deactivateds Routes
{
    // Route to get all Deactivateds
    app.get("/api/deactivateds", (req,res)=>{
    db.query("SELECT * FROM deactivateds",
        (err,result)=>{
        if(err) {
            //console.log(err)
            res.send(err)
    ...
    app.get("/api/deactivateds/:id/:year", (req,res)=>{
        const id = req.params.id;
        const year = req.params.year;
    
        const cod = `GA/DMEI Nº ${id} / ${year} - PT`;

        db.query("SELECT * FROM deactivateds WHERE id = ?", [cod],
            (err,result)=>{
            if(err) {
                //console.log(err)
                res.send(err)
    ...
    // Route to get all Deactivateds
    app.get("/api/deactivateds/this-year", (req,res)=>{
    db.query("SELECT * FROM deactivateds WHERE YEAR(deactivateds.date) = YEAR(CURDATE())",
        (err,result)=>{
        if(err) {
            //console.log(err)
            res.send(err)
    ...
    // Route to get Deactivateds document null
    app.get("/api/deactivateds/doc-null", (req,res)=>{

    db.query(`SELECT deactivateds.*, machines.num_serial as machine_serial, machines.model as machine_model FROM deactivateds JOIN machines ON machines.id = deactivateds.id_machine_d WHERE deactivateds.document IS NULL`, 
        (err,result)=>{
        if(err) {
            //console.log(err)
            res.send(err)
    ...
    // Route to get Deactivateds by limit
    app.get("/api/deactivateds/page=:page/perPage=:perPage/search=:search", (req,res)=>{
    ...
    db.query(`SELECT deactivateds.*, machines.num_serial as machine_serial, machines.model as machine_model FROM deactivateds JOIN machines ON machines.id = deactivateds.id_machine_d WHERE deactivateds.id LIKE ? OR machines.num_serial LIKE ? OR machines.model LIKE ? OR deactivateds.date LIKE ? LIMIT ?, ?`, [search, search, search, search, page, perPage], 
        (err,result)=>{
        if(err) {
            //console.log(err)
            res.send(err)
    ...
    app.post('/api/deactivateds/create/', (req,res)=> {

    const id = req.body.id;
    const machine = req.body.machine;
    const problem = req.body.problem;

    db.query("INSERT INTO deactivateds (id, id_machine_d, date, problem) VALUES (?,?,CURDATE(),?)",
        [id, machine, problem],
        (err,result)=>{
        if(err) {
            //console.log(err)
    ...
    app.patch('/api/deactivateds/create/doc/:id/:year', upload.single('file'), (req,res)=>{
        const id = req.params.id;
        const year = req.params.year;
        ...
        db.query("UPDATE deactivateds SET document = ? WHERE id = ?",
            [deactivate_doc, cod],
            (err,result)=>{
            if(err) {
                console.log(err);
...
// Substitution Routes
{
    ...
    // Route to Substitution Machine
    app.patch('/api/machines/:id/substitution', (req,res)=>{
        const id = req.params.id;
        const id_entity = req.body.id_entity;
...
// Loan Routes
{
    ...
    app.patch('/api/loans/create/doc-devolution/:id/:year', upload.single('file'), (req,res)=>{
        const id = req.params.id;
        const year = req.params.year;
        ...
        db.query("UPDATE loan SET document_return = ? WHERE id = ?",
            [document, cod],
            (err,result)=>{
            if(err) {
                //console.log(err);
    ...
    // Route to ADD a Devolution Date
    app.patch('/api/loans/:id/:year/date-devolution',(req,res)=>{
        const id = req.params.id;
        const year = req.params.year;
    
        const cod = `GA/DMEI Nº ${id} / ${year} - TE`;
    
            db.query("UPDATE loan SET date_return = CURDATE() WHERE id = ?", cod,
            (err,result)=>{
            if(err) {
                //console.log(err)
                res.send(err)
...
// Devolution Routes
{
    ...
    app.patch('/api/devolutions/create/doc/:id/:year', upload.single('file'), (req, res) => {
        ...
        const finalFilePath = `./uploads/${req.file.filename}`;
...
//Services Internal
{
    ...
    db.query("INSERT INTO service_internal (id_entity_si, problem, text_machines) VALUES (?,?,?)",
        [entity, problem, text_machines],
        (err,result)=>{
        if(err) {
            //console.log(err)
            res.send(err);
    ...
    app.patch('/api/internals/:id/doc/add', upload.single('file'), (req,res)=>{
        const id = req.params.id;
        ...
        app.patch('/api/inputs/:id/doc/remove',(req,res)=>{
            const id = req.params.id;
...
app.get("/api/devolutions/download/:filePath", (req, res) => {
...
/*
    httpsServer.listen(PORT, () => {
        console.log(`Servidor HTTPS em execução em https://localhost:${PORT}`);
    });
*/
