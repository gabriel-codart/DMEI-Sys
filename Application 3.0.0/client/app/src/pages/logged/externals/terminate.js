...
export default function TerminateExternal() {
    ...
    useEffect(() => {
        axios.get('http://10.10.136.100:3002/api/externals/not/terminateds').then((res) => {
    ...
    useEffect(() => {
        if (external !== null) {
            axios.get(`http://10.10.136.100:3002/api/externals/${external.value}`).then((res) => {
                console.log(res)
    ...
    const generatePDF = (insertId) => {
        confirmAlert({
            ...
            buttons: [
                ...
                label: 'Sim',
                onClick: () => {
                    navigate(`/dmei-sys/externals/terminateds/${insertId}/exit`);
                    }
                },
                {
                label: 'Não',
    ...
    const updateExternal = () => {
        if (user.value === null) {
            ...
            axios.patch(`http://10.10.136.100:3002/api/externals/${external.value}/terminate`, {
                user: user.value,
